import './OperatorsNetworkDiagram.css'
...
const PENDING_GAP = 8
const chainSpanToPending =
  (CANONICAL_COUNT - 1) * (BLOCK_WIDTH + BLOCK_GAP) + BLOCK_GAP * PENDING_GAP
...
export interface OperatorsNetworkDiagramProps {
  /** Label for the operator ring */
  operatorLabel?: string
  operatorSubLabel?: string
  /** Label for the user ring */
  userLabel?: string
  userSubLabel?: string
}

export function OperatorsNetworkDiagram({
  operatorLabel = 'Operators',
  operatorSubLabel = 'P2P mesh, relay transactions',
  userLabel = 'Users',
  userSubLabel = 'Submit transactions',
}: OperatorsNetworkDiagramProps = {}) {
  return (
    <div className="operators-network-diagram">
      <svg
        viewBox="0 0 600 600"
        className="operators-network-svg"
      ...
      >
        ...
        {Array.from({ length: CANONICAL_COUNT }, (_, i) => {
          ...
          return (
            <g key={`canonical-${i}`}>
              ...
              />
              <text x={x} y={CHAIN_Y} className="block-id">
                {i}
              </text>
              {i > 0 && (
                <line
        ...
        <line
          x1={
            CHAIN_START_X +
            (CANONICAL_COUNT - 1) * (BLOCK_WIDTH + BLOCK_GAP) +
            BLOCK_WIDTH / 2
          }
          y1={CHAIN_Y}
        ...
        <text
          x={PENDING_BLOCK_X}
          y={PENDING_BLOCK_Y}
          className="block-id block-id-pending"
        >
          ?
        </text>

        {/* Transaction flows: user to operator */}
        ...
        {/* Cyan transactions converge toward block producer */}
        {[
          [2, 0],
          [3, 0],
          [4, 0],
          ...
          return (
            <g key={`op-node-${i}`}>
              <circle
                ...
                className={
                  isProducer
                    ? 'operator-node operator-node-producer'
                    : 'operator-node'
                }
              />
              <circle
        ...
        <text
          x={CENTER_X}
          y={CENTER_Y - OPERATOR_RADIUS - 24}
          className="mesh-label mesh-label-header"
        >
          {operatorLabel}
        </text>
        <text
          x={CENTER_X}
          y={CENTER_Y - OPERATOR_RADIUS - 10}
          className="mesh-label mesh-label-sub"
        >
          {operatorSubLabel}
        </text>
        <text
          x={CENTER_X}
          y={CENTER_Y + USER_RADIUS + 10}
          className="user-label user-label-header"
        >
          {userLabel}
        </text>
        <text
          x={CENTER_X}
          y={CENTER_Y + USER_RADIUS + 24}
          className="user-label user-label-sub"
        >
          {userSubLabel}
        </text>
        <text
          x={CENTER_X}
          y={CHAIN_Y - BLOCK_HEIGHT / 2 - 8}
        ...
        <text
          x={PENDING_BLOCK_X}
          y={PENDING_BLOCK_Y - BLOCK_HEIGHT / 2 - 8}
          className="pending-label"
        >
          Pending
        </text>
      </svg>
      <p className="operators-network-legend">
